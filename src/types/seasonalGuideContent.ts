...
export interface SeasonalGuideContent {
  id: string;
  title: string;
  subtitle: string;
  introduction: string;
  ...
  travel_tips: SeasonalTip[];
  comparison_chart: ComparisonData;
  is_active: boolean;
  updated_at: string;
  created_at: string;
}

export interface SeasonalPark {
  id: string;
  name: string;
  slug: string;
  description: string;
...
export interface SeasonalPeriod {
  ...
  travel_rating: number; // 1-5 scale
...
export interface SeasonalActivity {
  ...
  seasonal_variations: ActivityVariation[];
}

export interface ActivityVariation {
  season_id: string;
  description: string;
  duration_adjustment: number; // hours
  difficulty_adjustment: number; // 1-5 scale
  special_notes: string;
}

export interface SeasonalTip {
  id: string;
  category: 'weather' | 'wildlife' | 'travel' | 'packing' | 'health';
  title: string;
  content: string;
  seasons: string[];
...
export const DEFAULT_SEASONAL_GUIDE: SeasonalGuideContent = {
  id: "main",
